...
public:
    bool canUse(const Item& item) const{
        if (item.isMagical() == false && getStrength() >= item.getWeight() && getLevel() >= item.getLevel()){
            return true;
        }
        else return false;
    }
...
public:
    bool canUse(const Item& item) const{
        if (( item.isMagical() == true || item.isMagical() == false) && getStrength() >= item.getWeight() && getLevel() >= item.getLevel()){
            return true;
        }
        else return false;
    }
